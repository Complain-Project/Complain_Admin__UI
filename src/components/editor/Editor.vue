<template>
    <section class="editor">
        <ckeditor id="editor" :editor="editor" v-model="contentEdit" :config="editorConfig">
        </ckeditor>
    </section>
</template>
...
export default {
    ...
    setup(props, context){
        let contentEdit = ref("")
        let contentProps = computed(() => toRaw(props.content))
        let create = computed(() => toRaw(props.isCreate))
        watch(create, () => {
          if(!create.value){
            contentEdit.value = contentProps.value
          }
        })

        watch(contentEdit, () => {
            context.emit("newContent", contentEdit.value)
        },{deep: true})
        ...
        return {
            ...
            editorConfig: {
                ...
                placeholder: 'Viết hoặc dán nội dung của bạn',
                style: {
                    definitions: [
                        {
                            name: 'Title',
