            ...
            <el-row class="action-wrap">
                ...
                <el-col :span="12" class="action-wrap--col action-btn">
                    <div class="action-wrap--col__btn">
                        <el-button v-if="hasPermission('add-admin')"
                                @click="openModalAddAdmin()"
                                class="btn-apply">
            ...
            <el-card class="list-admin">
                    ...
                    <el-row>
                        <el-col class="input">
                            <el-input
                                    ...
                                    clearable
                                    placeholder="Nhập vào tên, email hoặc số điện thoại để tìm kiếm"
                            >
                            </el-input>
                        </el-col>
                    </el-row>
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template #default="admin">
                            <div class="flex items-center">
                                <el-avatar :size="35" :src="admin.row.avatar ? admin.row.avatar : userDefault" />
                                <p class="admin-name ml-2">{{ admin.row.name }}</p>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column
                    ...
                    >
                        <template #default="admin">
                            <el-switch
                                    v-if="authUserComputed._id !== admin.row._id && !admin.row.is_admin"
                                    @change="changeStatus(admin.row.status, admin.row._id)"
                                    v-model="admin.row.status"
                                    :inactive-value="0"
                                    :active-value="1"
                                    active-color="#13ce66"
                    ...
                    >
                        <template #default="admin">
                            <div v-if="authUserComputed._id !== admin.row._id && !admin.row.is_admin">
                                <el-tooltip v-if="hasPermission('edit-admin')" class="item" effect="dark" content="Chỉnh sửa" placement="top">
                                    <a
                                ...
                                <el-tooltip v-if="hasPermission('edit-admin')" class="item" effect="dark" content="Đổi mật khẩu" placement="top">
                                    <a
                                ...
                                <el-tooltip v-if="hasPermission('delete-admin')" class="item" effect="dark" content="Xóa" placement="top">
                                    <a
                ...
                <el-col :span="24">
                    <div class="upload__wrapper">
                        <div class="upload__wrapper__upload relative w-4/12">
                            ...
                            <el-icon v-if="!id && avatar"
                                     @click="handleRemoveAvatar"
                                     class="el-icon-close avatar-delete-icon">
                                <Close />
                            </el-icon>
                        </div>
                    </div>
        ...
        >
            ...
            <template #footer>
                <el-button class="btn-close"
                           @click="closeModal()">Hủy</el-button>
                <el-button class="btn-apply"
                           @click="handleResetPassword()">Lưu</el-button>
            </template>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    setup: function () {
        ...
        const openModalAddAdmin = () => {
            resetForm()
            resetError()
            modalTitle.value = "Tạo nhân viên";
        ...
        const openModalUpdateAdmin = (admin) => {
            ...
            if (admin) {
                ...
                modalTitle.value = "Cập nhật nhân viên " + name.value;
        ...
        const handleCreateAdmin = () => {
            if (validateCreateData()) {
                ...
                data.append("status", Number(is_active.value))
                if(roleIds.value.length > 0){
                    roleIds.value.forEach((item, i) => {
                        data.append("role_id[" + i + "]", item)
                    })
                }
                if(fileUpload.value){
        ...
        const returnError = (error) => {
            ...
            if (Object.keys(errors).length > 0) {
                errorName.value = _.get(errors, 'name', "");
                errorEmail.value = _.get(errors, 'email', "");
                errorPassword.value = _.get(errors, 'password', "");
                errorPhone.value = _.get(errors, 'phone', "");
            } else if (status === 401) {
